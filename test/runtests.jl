...
@testset "GeodesicOnTheSphere" begin
    ...
    @test GeodesicOnTheSphere(f, g) isa GeodesicOnTheSphere{Int64}
    @test GeodesicOnTheSphere([f, f], [g, h]) isa GeodesicOnTheSphere{Int64}
    ...
    HDT!(u, [1, 2, 2.0], 0.0)
    @test u == [0, 0]
    @test string(H) == "Homotopy.GeodesicOnTheSphere{Float64} The homotopy is given by the spherical geodesic from start/|start| (t=1) to target/|target| (t=0). Here: target = [x+y+3.0z, y+2.0z], start = [x+y+3.0z, x+2.0z].\n"
